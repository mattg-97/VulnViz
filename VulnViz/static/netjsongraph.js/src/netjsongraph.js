    ...
    opts = d3._extend(
      {
        ...
        linkDistance: 50,
        linkStrength: 0.2,
        friction: 0.7, // d3 default
        chargeDistance: Infinity, // d3 default
        theta: 0.9, // d3 default
        gravity: 0.05,
        circleRadius: 8,
        labelDx: "0",
        labelDy: "-5.3em",
        ...
        onClickNode: function(n) {
          ...
          var overlay = d3.select(".njg-overlay"),
            ...
            html = '<form action='+action+ ' method="POST"><p name="ip_addr"><br/><b>id</b>: ' + n.id + '</p>';
          if (n.label) {
          ...
          if (n.properties) {
            for (var key in n.properties) {
              ...
              if (key == "data"){
                for (var x in n.properties[key]){
                  html+= "<p>Port "+n.properties[key][x]["port"]+ " is open" + "</p>"
                    // portString+= n.properties[key][x]["port"] + ",";
                    dataArray.push([n.properties[key][x]["port"],n.properties[key][x]["protocol"],n.properties[key][x]["service"]]);
                    portArray.push(n.properties[key][x]["port"])
          ...
          if (n.properties["open-ports"]) {
              html += `<input type="hidden" name="ip_input" value=${n.id}/>`
              html += `<input type="hidden" name="data" value=${dataArray}/>`
              html += `<input type="hidden" name="ports" value=${portArray}/>`

              html += '<button type="submit" class="btn btn-primary initiate" onclick="loaderFunction()">Advanced Scan</button></form><br/>'
          } else {
              html += '<p>No open ports</p></form><br/>'
          }
